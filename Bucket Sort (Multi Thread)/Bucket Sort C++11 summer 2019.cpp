...
void ManyBuckets::addItem(unsigned long item) {
  // TODO: Put the item in the appropraite bucket.
	auto bucket_width = ULONGMAX; //If it is just 1 bucket, it will stay this
	if (buckets > 1) {
		bucket_width = ULONGMAX / buckets + 1; //This will give us how wide our buckets are
	}
	auto index = item / bucket_width; //This is which bucket we are inserting the item
	arr[index].push_back(item);
}

// TODO:  Loop over every item and call addItem
void placeIntoBuckets() {
	for (int i = 0; i < listSize; i++) { //Run through the list to sort into buckets
...
void combineBuckets() {
	int index = 0; //This is the global list index
	for (int i = 0; i < numBuckets; i++) { //Loop through each bucket
		for (auto item : globalBuckets->getBucket(i)) { //ranged loop through the vectors
			list[index] = item; //Overwrite the list item at said index
...
void bucketSort(bool displayOutput, bool multiThreadingMode) {
  ...
  for (int t = 0; t < numThreadsToUse; t++) {
	  threads[t] = thread(sortEachBucket);
  }
  //Join each thread again
  for (int t = 0; t < numThreadsToUse; t++) {
	  threads[t].join();
  }
